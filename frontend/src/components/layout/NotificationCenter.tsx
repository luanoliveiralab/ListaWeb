...
export default function NotificationCenter() {
  ...
  const avisos = useMemo(() => {
    ...
    for (const orcamento of dados.orcamentos) { const gasto = atuais.filter((m) => m.tipo === "despesa" && m.categoria === orcamento.categoria).reduce((t, m) => t + Number(m.valor), 0); const percentual = gasto / Number(orcamento.valor); if (percentual >= .5) lista.push({ id: `o-${orcamento.id}`, titulo: percentual >= 1 ? "Orçamento atingido" : percentual >= .8 ? "Orçamento próximo do limite" : "Orçamento em atenção", detalhe: `${orcamento.categoria}: ${Math.round(percentual * 100)}% utilizado`, tipo: "alerta" }); }
