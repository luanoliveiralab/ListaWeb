...
export default function NotificationCenter() {
  ...
  return <div className="relative">
    <button className="icon-button relative border border-border bg-background" onClick={() => setAberto(!aberto)} aria-label="Notificações"><Bell size={18} />{naoLidos > 0 && <span aria-hidden="true" className="absolute -right-1 -top-1 flex size-4 items-center justify-center rounded-full bg-rose-500 text-[9px] font-bold text-white">{Math.min(9, naoLidos)}</span>}</button>
