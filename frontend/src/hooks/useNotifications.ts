...
function calcularAvisos(dados: DadosAvisos): Omit<AvisoApp, "lido">[] {
  ...
  for (const o of dados.orcamentos) { const gasto = atuais.filter((m) => m.tipo === "despesa" && m.categoria === o.categoria).reduce((t, m) => t + Number(m.valor), 0); const p = gasto / Number(o.valor); if (p >= .5) lista.push({ id: `o-${o.id}-${ano}-${mes + 1}`, titulo: p >= 1 ? "Orçamento atingido" : p >= .8 ? "Orçamento próximo do limite" : "Orçamento em atenção", detalhe: `${o.categoria}: ${Math.round(p * 100)}% utilizado`, tipo: "orcamento", href: "/planejamento", importante: p >= .8 }); }
