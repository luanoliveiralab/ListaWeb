...
import { ArrowLeft, CheckCircle2, ExternalLink, Gift, Globe2, Heart, Layers3, Link2, Mail, Rocket, ShieldCheck, UserRound } from "lucide-react";
...
const contatos = [
  { nome: "GitHub", href: process.env.NEXT_PUBLIC_ABOUT_GITHUB ?? "https://github.com/luanoliveiralab/ListaWeb", icon: Link2 },
  { nome: "LinkedIn", href: process.env.NEXT_PUBLIC_ABOUT_LINKEDIN, icon: Globe2 },
  { nome: "Portfólio", href: process.env.NEXT_PUBLIC_ABOUT_PORTFOLIO, icon: ExternalLink },
  { nome: "Contato", href: process.env.NEXT_PUBLIC_ABOUT_EMAIL ? `mailto:${process.env.NEXT_PUBLIC_ABOUT_EMAIL}` : undefined, icon: Mail },
].filter((item): item is { nome: string; href: string; icon: typeof Link2 } => Boolean(item.href));
...
export default function SobrePage() {
  return <main className="min-h-screen bg-background text-foreground">
    <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10">
      ...
      <aside className="space-y-6"><section className="surface p-6"><span className="flex size-11 items-center justify-center rounded-xl bg-violet-500/10 text-violet-600"><UserRound size={21} /></span><h2 className="mt-4 text-xl font-semibold">Sobre o criador</h2><p className="mt-3 text-sm leading-6 text-muted-foreground">Desenvolvido por Luan Oliveira como um projeto independente, unindo produto, design e tecnologia para resolver problemas cotidianos de organização financeira.</p>{contatos.length > 0 && <div className="mt-5 flex flex-wrap gap-2">{contatos.map(({ nome, href, icon: Icon }) => <a key={nome} href={href} target={href.startsWith("http") ? "_blank" : undefined} rel="noreferrer" className="button-secondary"><Icon size={16} /> {nome}</a>)}</div>}</section>
