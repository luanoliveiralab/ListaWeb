...
import { ArrowLeft, CheckCircle2, ExternalLink, Gift, Globe2, Heart, Layers3, Link2, Mail, Rocket, ShieldCheck } from "lucide-react";
...
const contatos = [
  ...
  { nome: "LinkedIn", href: process.env.NEXT_PUBLIC_ABOUT_LINKEDIN ?? "https://www.linkedin.com/in/luanoliveira-ld", icon: Globe2 },
...
export default function SobrePage() {
  return <main className="min-h-screen bg-background text-foreground">
    <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10">
      ...
      <aside className="space-y-6"><section className="surface p-6"><div className="flex items-center gap-4"><Image src="/luan-oliveira.jpeg" alt="Foto de Luan Oliveira" width={96} height={96} className="size-24 shrink-0 rounded-2xl object-cover object-top shadow-md ring-1 ring-border" /><div><p className="text-sm font-medium text-primary">Criador do ListaWeb</p><h2 className="mt-1 text-xl font-semibold">Luan Oliveira</h2><p className="mt-1 text-xs text-muted-foreground">Desenvolvedor full-stack</p></div></div><p className="mt-5 text-sm leading-6 text-muted-foreground">Desenvolvido por Luan Oliveira como um projeto independente, unindo produto, design e tecnologia para resolver problemas cotidianos de organização financeira.</p>{contatos.length > 0 && <div className="mt-5 flex flex-wrap gap-2">{contatos.map(({ nome, href, icon: Icon }) => <a key={nome} href={href} target={href.startsWith("http") ? "_blank" : undefined} rel="noopener noreferrer" className="button-secondary"><Icon size={16} /> {nome}</a>)}</div>}</section>
